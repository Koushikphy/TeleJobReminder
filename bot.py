...
class DataBase:
    ...
    def listJobs(self, userID, option="d"):
    ...
    def getJobDetail(self,userId, jobID): #WIP
        with self.con:
            with self.con.cursor() as cur:
                ...
                cur.execute("SELECT job,host,directory,status,added,closed from JOBINFO where jobID=%s and userID=%s",(jobID,userId))
                info = cur.fetchone()
                # print(info)
                return dedent(f'''
                    Job Details:
...
def send_welcome(user):
    ...
    bot.send_message(user, f"Hi there <b>{user}</b>. "
        ...
        f"Your id is <b>{user}</b>. Use this when submitting jobs.")
    bot.send_message(user,"Download and run the "
...
def helpMessage(user):
    bot.send_message(user," Contact <a href='https://t.me/Koushikphy'>Koushik Naskar (Admin)</a> or "
        "check out this <a href='https://github.com/Koushikphy/TeleJobReminder'>Github Repo</a> or for further queries.")



def send_userinfo(user):
    print(f'Information requested for {user}')
    bot.send_message(user, f"Your id is <b>{user}</b>. Use this when submitting jobs")
...
def allCommnad(message):
    ...
    isRegisteredUser = db.checkIfRegisteredID(user)
    ...
    if text=="/help":
        ...
        bot.send_message(user,db.listJobs(user,))
        ...
        bot.send_message(user,db.listJobs(user,option='r'))
